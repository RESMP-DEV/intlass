...
void check_sm86(torch::Tensor const& tensor) {
    ...
    TORCH_CHECK(props.major == 8 && props.minor == 6,
                "matmul_host_int8_bf16_tn requires an SM86 CUDA device, got sm_",
                props.major,
                props.minor);
}
...
// Source lineage: contrib/int4_kernel/kernel/w4a8_gemm_sm86.cu documents the
// Ampere W4A8 INT8 activation path this INT8 correctness kernel is replacing.
// Future Tensor Core path evidence: mma.sync.aligned.m16n8k32.row.col.s32.s8.s8.s32
template <typename AScaleT, typename BScaleT>
__global__ void matmul_int8_bf16_tn_kernel(int8_t const* __restrict__ A,
    ...
    for (int64_t k = 0; k < K; ++k) {
        int32_t const a = static_cast<int32_t>(A[m * K + k]);
        int32_t const b = static_cast<int32_t>(B[n * K + k]);
        acc += a * b;
    }
...
template <typename AScaleT, typename BScaleT>
void launch_matmul_int8_bf16_tn(torch::Tensor& out,
    ...
    matmul_int8_bf16_tn_kernel<AScaleT, BScaleT><<<grid, block, 0, stream>>>(
        A.data_ptr<int8_t>(),
        B.data_ptr<int8_t>(),
...
void dispatch_b_scale(torch::Tensor& out,
    ...
    if (B_scale.scalar_type() == at::kFloat) {
        launch_matmul_int8_bf16_tn<AScaleT, float>(out, A, B, A_scale, B_scale, stream);
    } else if (B_scale.scalar_type() == at::kHalf) {
        launch_matmul_int8_bf16_tn<AScaleT, c10::Half>(out, A, B, A_scale, B_scale, stream);
    } else {
        launch_matmul_int8_bf16_tn<AScaleT, c10::BFloat16>(out, A, B, A_scale, B_scale, stream);
    }
}
...
void matmul_host_int8_bf16_tn(torch::Tensor& out,
                              torch::Tensor const& A,
                              torch::Tensor const& B,
                              torch::Tensor const& A_scale,
                              torch::Tensor const& B_scale) {
    ...
    TORCH_CHECK(A.scalar_type() == at::kChar, "A must be torch.int8");
    TORCH_CHECK(B.scalar_type() == at::kChar, "B must be torch.int8");
    ...
    torch::checkAllSameGPU("matmul_host_int8_bf16_tn",
                           {{out, "out", 0},
                            {A, "A", 1},
                            {B, "B", 2},
                            {A_scale, "A_scale", 3},
