...
BF16_LABEL = "INT8 SM86 BF16 PyTorch baseline"
INT8_LABEL = "INT8 SM86 matmul_int8_bf16_tn"
DEFAULT_SHAPES = (
    (16, 512, 512),
    (64, 512, 512),
    (128, 1024, 1024),
...
def require_sm86(device: int) -> None:
    if not torch.cuda.is_available():
        raise SystemExit("CUDA is required for the INT8 SM86 benchmark")

    torch.cuda.set_device(device)
    capability = torch.cuda.get_device_capability(device)
    if capability < (8, 6):
        raise SystemExit(
            "INT8 SM86 benchmark requires device capability at least "
            f"(8, 6), got {capability}"
        )
    if capability != (8, 6):
        raise SystemExit(
            "INT8 SM86 benchmark must run on an SM86 CUDA device, got "
...
def run_shape(m: int, n: int, k: int, warmup: int, iters: int) -> tuple[float, float]:
    ...
    a_scale = row_scale(a)
    b_scale = row_scale(b)
    a_int8 = quantize_int8(a, a_scale)
    b_int8 = quantize_int8(b, b_scale)

    bf16_ms = time_cuda(lambda: torch.nn.functional.linear(a, b), warmup, iters)
    int8_ms = time_cuda(
        lambda: matmul_int8_bf16_tn(a_int8, b_int8, a_scale, b_scale),
        warmup,
        iters,
    )
    return bf16_ms, int8_ms


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="INT8 SM86 GEMM benchmark")
    parser.add_argument(
        "--shape",
        action="append",
        type=parse_shape,
...
def main() -> None:
    ...
    print("INT8 SM86 benchmark")
    print(f"{'M':>8} {'N':>8} {'K':>8} {BF16_LABEL + ' ms':>36} {INT8_LABEL + ' ms':>36}")
    for m, n, k in shapes:
        bf16_ms, int8_ms = run_shape(m, n, k, args.warmup, args.iters)
        print(f"{m:8d} {n:8d} {k:8d} {bf16_ms:36.4f} {int8_ms:36.4f}")
